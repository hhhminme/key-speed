...
router.get('/:text', async (req: Request<{ text: string }>, res: Response): Promise<void> => {
  const text = req.params.text as string;

  if (!isKoreanText(text)) {
    res.status(400).json({ error: 'Invalid text. Must be Korean syllables (U+AC00~U+D7A3), max 10 characters.' });
    return;
  }
